...
def scale_button(button, screen_width):
    return pygame.transform.scale(
        button,
        (
            int(screen_width * 0.32),
            int(button.get_height() * (screen_width * 0.32) / button.get_width()),
        ),
    )
...
def load_buttons(screen_width):
    ...
    option = scale_button(pygame.image.load(r'assets\StartMenu\Buttons\optionen.png'), screen_width)
    credits = scale_button(pygame.image.load(r'assets\StartMenu\Buttons\credits.png'), screen_width)
    quit = scale_button(pygame.image.load(r'assets\StartMenu\Buttons\quit.png'), screen_width)
...
def make_start_menu(screen, Game_name, option_lines, credits_lines):
    ...
    while running:
        ...
        for i, button in enumerate(buttons):
            screen.blit(
                button,
                (
                    (screen_width - button.get_width()) // 2,
